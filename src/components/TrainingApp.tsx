...
export function TrainingApp() {
  ...
  const [pendingEntries, setPendingEntries] = useState<{ name: string | null; email: string }[]>([]);
  const [pendingFormError, setPendingFormError] = useState("");
  const [assigning, setAssigning] = useState(false);
  const [assignResult, setAssignResult] = useState<{ assigned: number; skipped: number; emailFailures: string[] } | { error: string } | null>(null);
  ...
  const PENDING_EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

  function addPendingEntry() {
    const email = pendingEmail.trim().toLowerCase();
    if (!PENDING_EMAIL_RE.test(email)) {
      setPendingFormError("Enter a valid email address.");
      return;
    }
    if (pendingEntries.some((e) => e.email === email)) {
      setPendingFormError("That email is already in the list below.");
      return;
    }
    setPendingEntries((list) => [...list, { name: pendingName.trim() || null, email }]);
    setPendingName("");
    setPendingEmail("");
    setPendingFormError("");
  }

  function removePendingEntry(email: string) {
    setPendingEntries((list) => list.filter((e) => e.email !== email));
  }
  ...
  async function deleteAssignment(id: string) {
    if (!window.confirm("Remove this assignment? They'll stop receiving reminders.")) return;
    setDeletingAssignmentId(id);
    try {
      const res = await fetch(`/api/assignments/${id}`, { method: "DELETE" });
      if (!res.ok) {
        window.alert("Could not remove that assignment. Try again.");
        return;
      }
      setAssignments((list) => (list ? list.filter((a) => a.id !== id) : list));
    } catch {
      window.alert("Could not remove that assignment. Check your connection and try again.");
    } finally {
          ...
          {screen === "facilitatorDashboard" && (
            <>
              ...
              <p style={{ opacity: 0.7, maxWidth: "65ch", marginBottom: "var(--space-4)" }}>
                Add each person below, then send. They get an email with a link to start now, then a
                reminder every few days until they complete it.
              </p>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr auto", gap: "var(--space-3)", alignItems: "end", marginBottom: "var(--space-3)" }}>
                <div className="field">
                  <label htmlFor="cq-pending-name">Name (optional)</label>
                  <input
                    className="input"
                    id="cq-pending-name"
                    value={pendingName}
                ...
                <div className="field">
                  <label htmlFor="cq-pending-email">Email</label>
                  <input
                    className="input"
                    id="cq-pending-email"
                    type="email"
                    ...
                    placeholder="jane.doe@chcrichmond.org"
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        e.preventDefault();
                        addPendingEntry();
                ...
                <button type="button" className="btn btn-secondary" onClick={addPendingEntry}>
                  Add
                </button>
              </div>
              {pendingFormError && (
                <p style={{ color: "var(--color-accent-700)", fontSize: 13, marginTop: -4, marginBottom: "var(--space-3)" }}>{pendingFormError}</p>
              )}
              ...
              {pendingEntries.length > 0 && (
                <div style={{ display: "flex", flexWrap: "wrap", gap: "var(--space-2)", marginBottom: "var(--space-4)" }}>
                  {pendingEntries.map((e) => (
                    <div
                      key={e.email}
                      style={{ display: "flex", alignItems: "center", gap: 8, background: "var(--color-neutral-100)", borderRadius: "var(--radius-lg)", padding: "6px 8px 6px 14px", fontSize: 13 }}
                    >
                      <span>{e.name ? `${e.name} — ${e.email}` : e.email}</span>
                      <button type="button" className="btn-icon" title="Remove" onClick={() => removePendingEntry(e.email)}>✕</button>
                    </div>
                  ))}
                </div>
              )}

              <button type="button" className="btn btn-primary" disabled={assigning || pendingEntries.length === 0} onClick={submitAssignments}>
                {assigning ? "Sending…" : `Send Assignment${pendingEntries.length === 1 ? "" : "s"}${pendingEntries.length > 0 ? ` (${pendingEntries.length})` : ""}`}
              </button>
              ...
              {assignResult && "assigned" in assignResult && (
                <p style={{ fontSize: 13, marginTop: 8, opacity: 0.85 }}>
                  ...
                  {assignResult.skipped > 0 && ` Skipped ${assignResult.skipped} invalid ${assignResult.skipped === 1 ? "entry" : "entries"}.`}
